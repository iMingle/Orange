 ...
 * @author mingle
 */
public final class StandardDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static final Object mouseLock;
    private static final Object keyLock;
    ...
    private static final StandardDraw std;
    private static JFrame frame;
    private static boolean mousePressed;
    private static double mouseX;
    private static double mouseY;
    private static final LinkedList<Character> keysTyped;
    private static final TreeSet<Integer> keysDown;
    ...
    public static void point(double x, double y) {
        ...
        if (scaledPenRadius <= 1.0F) {
            ...
            offscreen.fill(new java.awt.geom.Ellipse2D.Double(xs - (double) (scaledPenRadius / 2.0F),
                    ys - (double) (scaledPenRadius / 2.0F), (double) scaledPenRadius, (double) scaledPenRadius));
        }
    ...
    public static void arc(double x, double y, double radius, double angle1, double angle2) {
        if (radius < 0.0D) {
            ...
            if (ws <= 1.0D && hs <= 1.0D) {
                ...
                offscreen.draw(new java.awt.geom.Arc2D.Double(xs - ws / 2.0D, ys - hs / 2.0D, ws, hs, angle1,
                        angle2 - angle1, 0));
            }
    ...
    private static Image getImage(String filename) {
        if (filename == null) {
            ...
            if (icon.getImageLoadStatus() != 8) {
                url = StandardDraw.class.getResource(filename);
                if (url != null) {
                    icon = new ImageIcon(url);
                }
            }
    ...
    public static void picture(double x, double y, String filename) {
        ...
        if (ws >= 0 && hs >= 0) {
            offscreen.drawImage(image, (int) Math.round(xs - (double) ws / 2.0D),
                    (int) Math.round(ys - (double) hs / 2.0D), (ImageObserver) null);
            draw();
        } else {
        ...
        if (scaledWidth < 0.0D) {
            ...
            if (ws >= 0.0D && hs >= 0.0D) {
                if (ws <= 1.0D && hs <= 1.0D) {
                    ...
                    offscreen.drawImage(image, (int) Math.round(xs - ws / 2.0D), (int) Math.round(ys - hs / 2.0D),
                            (int) Math.round(ws), (int) Math.round(hs), (ImageObserver) null);
                }
